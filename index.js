...
function checkLevelUpAndChangeXP(message, currentSaveData, xpGain) {
  ...
  if (currentXP + xpGain >= xpToNextLevel) {
    ...
    currentSaveData[authorId]["xpToNextLevel"] =
      xpToNextLevel * xpToNextLevelMultiplier;
  ...
  if (
    parseInt(getJsonData(saveDataPath)[authorId]["currentXP"]) >=
    parseInt(getJsonData(saveDataPath)[authorId]["xpToNextLevel"])
  ) {
  ...
  if (Date.now() < trainingCooldown) {
    ...
    if (previousLevel != getJsonData(saveDataPath)[authorId]["level"])
      message.channel.send(
        `${mentionUser(
          authorId
        )} has leveled up from level ${previousLevel} to level ${
          getJsonData(saveDataPath)[authorId]["level"]
        }!`
      );
...
function partyView(message) {
  ...
  var printStr =
    "Party leader is: " +
    mentionUser(party[playerPartyId]["leader"]) +
    ", The current members of your party consist of: ";
...
function partyLeave(message) {
  ...
  if (party[playerPartyId]["leader"] == message.author.id) {
    ...
    if (members.length == 0) {
      ...
      message.channel.send(
        mentionUser(members[0]) + " is the new party leader."
      );
    }
  } else {
...
function partyJoin(message, targetPlayer) {
  ...
  if (!isNull(playerSavePartyId)) {
    message.channel.send(
      "You're already in a party, please leave your current one first."
    );
    return;
  }
...
function partyInvite(message, targetPlayer) {
  ...
  if (isNull(targetPlayer)) {
    message.channel.send(
      `'${targetPlayer}' is not a valid player. Please enter an existing player.`
    );
    return;
  }
  ...
  if (isNull(invitersPartyId)) {
    message.channel.send(
      `${mentionUser(
        authorId
      )} is not currently in a party. Join one first before using ${prefix}invite`
    );
    return;
  }
  ...
  message.channel.send(
    `${mentionUser(authorId)} has invited ${mentionUser(
      targetPlayer
    )} to their party.`
  );
  ...
  collector.on("collect", (replyMessage) => {
    if (replyMessage.content.toLowerCase() == "accept") {
      ...
      replyMessage.channel.send(
        `${mentionUser(targetPlayer)} has declined ${mentionUser(
          authorId
        )}'s invite.`
      );
      ...
      replyMessage.channel.send(
        `Please type 'accept' or 'decline', ${mentionUser(targetPlayer)}.`
      );
    }
  });
  ...
  collector.on("end", (collected, reason) => {
    if (reason !== "accepted" && reason !== "declined")
      message.channel.send(
        `${mentionUser(targetPlayer)} did not respond to ${mentionUser(
          authorId
        )}'s invite.`
      );
  });
